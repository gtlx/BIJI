  ...
  return (
    <aside className={`sidebar ${collapsed ? 'collapsed' : ''}`}>
      <div className="sidebar-header">
        ...
        <button className="sidebar-collapse-btn" onClick={onToggleCollapse} title={collapsed ? '展开' : '折叠'}>
            ...
            {buttons.filter(b => b.visible).map((button) => (
              ...
              >
                <button
                  ...
                  onClick={() => !editingButtons && (button.id === 'files' ? onSelectFolder(null) : onToggleButton(button.id))}
                  title={button.label}
                ...
                {editingButtons && <span className="drag-handle">⋮⋮</span>}
              </div>
            ))}
            <button className="sidebar-btn add-btn" onClick={() => setEditingButtons(!editingButtons)} title={editingButtons ? '完成' : '编辑'}>
              <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor">
                {editingButtons ? (
              ...
              <div className="breadcrumb">
                {breadcrumb.map((item, index) => (
                  <span key={item.id ?? 'root'}>
                    ...
                    <button className={`breadcrumb-item ${item.id === selectedFolderId ? 'active' : ''}`} onClick={() => onSelectFolder(item.id)}>
                      {item.name}
                    </button>
                  </span>
                ))}
            ...
            <div className="nav-section">
              ...
              {subFolders.length > 0 ? (
                subFolders.map(folder => (
                  <button key={folder.id} className="nav-item" onClick={() => onSelectFolder(folder.id)} onDoubleClick={() => toggleFolder(folder.id)}>
                    <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
